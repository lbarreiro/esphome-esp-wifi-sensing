...
class AdaptiveMotionDetector {
 public:
  ...
  void set_persistence_samples(uint8_t samples) {
    if (samples < 1) samples = 1;
    if (samples > kPersistenceWindowSize) samples = kPersistenceWindowSize;
    this->persistence_required_samples_ = samples;
  }
  ...
  AdaptiveMotionDetectorResult update(uint32_t metric, uint32_t now_ms) {
    ...
    if (!ready) {
      ...
      if (candidate && persistence_on) {
        if (this->debounce_count_ < this->debounce_samples_) this->debounce_count_++;
    ...
    return this->result_(candidate);
  }

  bool persistence_on() const {
    return this->persistence_valid_samples_ == kPersistenceWindowSize &&
           this->persistence_elevated_count_ >= this->persistence_required_samples_;
  }

  float persistence() const {
    if (this->persistence_valid_samples_ == 0) return 0.0f;
  ...
  float adaptive_threshold() const { return this->adaptive_threshold_enabled_ ? this->adaptive_threshold_value_() : static_cast<float>(this->fixed_threshold_); }
 ...
 protected:
  void update_baseline_(uint32_t metric, uint32_t elapsed_ms) {
    ...
    const uint32_t time_ms = sample >= this->baseline_mean_ ? this->baseline_rise_time_ms_ : this->baseline_fall_time_ms_;
    float alpha = 1.0f;
    if (time_ms > 0) {
  ...
  uint8_t persistence_required_samples_{7};
