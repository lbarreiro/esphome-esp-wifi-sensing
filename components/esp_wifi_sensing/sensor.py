...
CONF_REJECTED_PEAK = "rejected_peak"
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_REJECTED_PEAK): sensor.sensor_schema(
                accuracy_decimals=2,
                state_class=STATE_CLASS_MEASUREMENT,
                entity_category=ENTITY_CATEGORY_DIAGNOSTIC,
            ),
...
async def to_code(config):
    ...
    if CONF_REJECTED_PEAK in config:
        sens = await sensor.new_sensor(config[CONF_REJECTED_PEAK])
        cg.add(parent.set_rejected_peak_sensor(sens))
