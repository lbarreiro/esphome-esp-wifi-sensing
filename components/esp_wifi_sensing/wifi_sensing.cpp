...
void ESPWiFiSensing::loop() {
  ...
  if (this->new_csi_sample_) {
    ...
    if (this->have_previous_sample_) {
      const uint32_t variation = metric >= this->previous_csi_metric_
                                     ? metric - this->previous_csi_metric_
  ...
  if (this->variation_samples_ > 0) {
    average_variation = this->variation_sum_ / this->variation_samples_;
  }
...
bool ESPWiFiSensing::start_ping_() {
  ...
  ESP_LOGI(TAG, "Router gateway: " IPSTR, IP2STR(&ip_info.gw));
...
void ESPWiFiSensing::csi_callback_(void *ctx, wifi_csi_info_t *data) {
  if (ctx == nullptr || data == nullptr || data->buf == nullptr || data->len == 0) {
    return;
  }
